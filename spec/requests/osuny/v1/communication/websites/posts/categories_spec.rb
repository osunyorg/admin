...
RSpec.describe 'Communication::Website::Post::Category' do
  fixtures :all

  path '/communication/websites/{website_id}/posts/categories' do
    get "Lists a website's post categories" do
      tags 'Communication::Website::Post::Category'
      security [{ api_key: [] }]
      ...
      response '200', 'Successful operation' do
        schema type: :array, items: { '$ref' => '#/components/schemas/communication_website_post_category' }
        run_test!
      end
    ...
    post 'Creates a post category' do
      tags 'Communication::Website::Post::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
      ...
      parameter name: :communication_website_post_category, in: :body, type: :object, schema: {
        type: :object,
        properties: {
          category: {
            '$ref': '#/components/schemas/communication_website_post_category'
          }
        },
        required: [:category]
      }
      let(:communication_website_post_category) {
        {
          category: {
            migration_identifier: 'post-category-from-api-1',
            localizations: {
              fr: {
                migration_identifier: 'post-category-from-api-1-fr',
                ...
                blocks: [
                  {
                    migration_identifier: 'post-category-from-api-1-fr-block-1',
      ...
      response '201', 'Successful creation' do
        it 'creates a post category and its localization', rswag: true, vcr: true do |example|
  ...
  path '/communication/websites/{website_id}/posts/categories/upsert' do
    post 'Upsert post categories' do
      tags 'Communication::Website::Post::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
      ...
      let(:categories) {
        test_category = communication_website_post_categories(:test_category)
        test_category_l10n = communication_website_post_category_localizations(:test_category_fr)
        {
          categories: [
            {
              migration_identifier: 'post-category-from-api-1',
      ...
      response '200', 'Successful upsertion' do
        it 'creates a post category and updates another with their localizations', rswag: true, vcr: true do |example|
          assert_difference ->{ Communication::Website::Post::Category.count } => 1, ->{ Communication::Website::Post::Category::Localization.count } => 1 do
            submit_request(example.metadata)
            assert_response_matches_metadata(example.metadata)
          end
  ...
  path '/communication/websites/{website_id}/posts/categories/{id}' do
    get 'Shows a post category' do
      tags 'Communication::Website::Post::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
      ...
      let(:id) { communication_website_post_categories(:test_category).id }
    ...
    patch 'Updates a post category' do
      tags 'Communication::Website::Post::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
      ...
      response '200', 'Successful update' do
        run_test! do |response|
          assert_equal("Mon nouveau nom", communication_website_post_category_localizations(:test_category_fr).reload.name)
        end
      end
    ...
    delete 'Deletes a post category' do
      tags 'Communication::Website::Post::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
