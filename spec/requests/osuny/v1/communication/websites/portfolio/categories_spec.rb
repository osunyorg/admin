...
RSpec.describe 'Communication::Website::Portfolio::Category' do
  fixtures :all

  path '/communication/websites/{website_id}/portfolio/categories' do
    get "Lists a website's portfolio categories" do
      tags 'Communication::Website::Portfolio::Category'
      security [{ api_key: [] }]
      ...
      parameter name: :per_page, in: :query, schema: { type: :integer, default: 25, maximum: 100 }, description: 'Number of items per page', required: false
      ...
      response '200', 'Successful operation' do
        schema type: :array, items: { '$ref' => '#/components/schemas/communication_website_portfolio_category' }
        run_test!
      end
    ...
    post 'Creates a portfolio category' do
      tags 'Communication::Website::Portfolio::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
      ...
      parameter name: :communication_website_portfolio_category, in: :body, type: :object, schema: {
        ...
        properties: {
          category: {
            '$ref': '#/components/schemas/communication_website_portfolio_category'
          }
        },
        required: [:category]
      }
      let(:communication_website_portfolio_category) {
        {
          category: {
            migration_identifier: 'portfolio-category-from-api-1',
            localizations: {
              fr: {
                migration_identifier: 'portfolio-category-from-api-1-fr',
                ...
                blocks: [
                  {
                    migration_identifier: 'portfolio-category-from-api-1-fr-block-1',
      ...
      response '201', 'Successful creation' do
        it 'creates a portfolio category and its localization', rswag: true do |example|
          assert_difference ->{ Communication::Website::Portfolio::Category.count } => 1, ->{ Communication::Website::Portfolio::Category::Localization.count } => 1 do
  ...
  path '/communication/websites/{website_id}/portfolio/categories/upsert' do
    post 'Upsert portfolio categories' do
      tags 'Communication::Website::Portfolio::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
      ...
      let(:categories) {
        test_category = communication_website_portfolio_categories(:test_category)
        test_category_l10n = communication_website_portfolio_category_localizations(:test_category_fr)
        {
          categories: [
            {
              migration_identifier: 'portfolio-category-from-api-1',
      ...
      response '200', 'Successful upsertion' do
        it 'creates a portfolio category and updates another with their localizations', rswag: true do |example|
          assert_difference ->{ Communication::Website::Portfolio::Category.count } => 1, ->{ Communication::Website::Portfolio::Category::Localization.count } => 1 do
            assert_enqueued_jobs 1, only: Api::AttachFeaturedImageFromUrlJob do
              submit_request(example.metadata)
              assert_response_matches_metadata(example.metadata)
  ...
  path '/communication/websites/{website_id}/portfolio/categories/{id}' do
    get 'Shows a portfolio category' do
      tags 'Communication::Website::Portfolio::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
    ...
    patch 'Updates a portfolio category' do
      tags 'Communication::Website::Portfolio::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
      ...
      response '200', 'Successful update' do
        run_test! do |response|
          assert_equal("Mon nouveau nom", communication_website_portfolio_category_localizations(:test_category_fr).reload.name)
        end
      end
    ...
    delete 'Deletes a portfolio category' do
      tags 'Communication::Website::Portfolio::Category'
      security [{ api_key: [] }]
      let("X-Osuny-Token") { university_apps(:default_app).token }
