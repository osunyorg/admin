class Api::Osuny::Communication::Websites::Portfolio::CategoriesController < Api::Osuny::Communication::Websites::ApplicationController
  ...
  def upsert
    ...
    permitted_categories_params.each_with_index do |permitted_category_params, index|
      category = website.portfolio_categories.find_by(migration_identifier: permitted_category_params[:migration_identifier])
      if category.present?
        ...
        category = website.portfolio_categories.build(permitted_category_params)
        if category.save
  ...
  def category_params
    @category_params ||= begin
      permitted_params = params.require(:category)
                          .permit(
                            :migration_identifier, :parent_id, :position, :is_taxonomy, :bodyclass, localizations: {}
                          ).merge(
  ...
  def category_params_for_upsert(category_params)
    ...
    category = website.portfolio_categories.find_by(migration_identifier: permitted_params[:migration_identifier])
