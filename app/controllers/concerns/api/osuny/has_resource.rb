module Api::Osuny::HasResource
  ...
  def load_resources
    self.resources = paginate(resource_list.includes(:localizations))
  end

  # :pages
  def resources_name
    raise NoMethodError, "You must implement the `resources_name` method in #{self.class.name}"
  end
  ...
  def load_resource
    return self.resource if self.resource.present?
    self.resource = resource_list.find_by(id: params[:id])
    self.resource ||= resource_list.find_by!(migration_identifier: params[:id])
    self.resource
  end
