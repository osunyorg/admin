class Migrations::BlockTitlesToFiles
  ...
  def self.migrate
    Communication::Block.where(template_kind: [:files, :sound]).with_deleted.find_each do |block|
      ...
      when :sound
        report_title(block.template.communication_file, block.template.title, block.language)
      when :files
        block.template.elements.each do |element|
          report_title(element.communication_file, element.title, block.language)
        end
      end
    end
  end
  ...
  def self.report_title(communication_file, title, language)
    ...
    return unless file_l10n.name == file_l10n.original_guessed_name
